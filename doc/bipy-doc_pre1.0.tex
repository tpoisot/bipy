...
To optimize performance, \texttt{bipy} makes an intensive use of the \texttt{numpy} and \texttt{scipy} packages.
...
\section{Robustness to extinction}

The extinction robustness analysis used notably by \cite{Memmott2004} are implemented in \texttt{bipy}. Three scenarios of extinction are available: best case (increasing degree, species with a low degree go first), random, and worst case (decreasing degree, highly-connected species go first).

\chapter{Null models}

\texttt{bipy} proposes (as of now) three main classes of different null models. This chapter presents the way they work and the specificity of each. The purpose of a null model is to use some features of the original network to create a given number of random networks. All null models implemented in \texttt{bipy} share the property that the networks they return will have the exact same size than the original network. A consequence of this is that there are no species with no interactions -- for this reason, networks with a low connectance cannot be meaningfully randomized, and \texttt{bipy} will issue a warning. For networks which can be randomized, but still have a low connectance, the null model will likely require a large number of iterations to find a solution with all rows and columns with at least one interaction, so the analysis might take a long time.

\section{The wrapper for null models}

\section{Null models\label{sec:nulls}}
...
\subsection{Null model 1 -- Bernoulli fill (\texttt{null\_1})}
...
\subsection{Null model 2 -- interaction average (\texttt{null\_2})}
...
\subsection{Null model 3 -- row averages (\texttt{null\_3row})}

\subsection{Null model 3 -- column averages (\texttt{null\_3col})}

\section{Tests of the deviation of common measures}

The general approach for testing the deviation of the sructure from a null expectation follows the same procedure. First, we measure $N$ the metric of interest (e.g. modularity or nestedness), then we conduct a given number of replicates of one of the four proposed null models (see section \ref{sec:nulls}), and measure $N'$ (the same metric on the random network) for each. The deviation between the original structure and itsandom expectation is given by $N-N'$, whose mean should be 0 if the structure is random. To assess any deviation from the random expectation, we use a one-sample two-tailed t-test.
